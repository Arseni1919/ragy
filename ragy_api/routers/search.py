...
from ragy_api.services.search_service import web_search, yfinance_search, bright_data_search
...
@router.post("/bright_data", response_model=WebSearchResponse)
async def search_bright_data_endpoint(request: WebSearchRequest):
    """Search the web using Bright Data"""
    return await bright_data_search(request.query, request.max_results)
